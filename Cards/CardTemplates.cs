...
namespace IMK.SettingsUI.Cards
{
    public static partial class CardTemplates
    {
        ...
        public static GameObject Bind(ICardModel model, GameObject existing)
        {
            ...
            {
                var err = new MarkdownCardModel{ Id=model?.Id+":bind_error", Title="Card Error", Markdown=$"ø®∆¨∞Û∂® ß∞‹: `{ex.Message}`" };
        ...
        internal static GameObject BindSettingStrategy(ICardModel model, GameObject existing)
        {
            ...
            var err = new MarkdownCardModel{ Id=model.Id+":unsupported_setting", Title="Unsupported Setting", Markdown=$"Œ¥◊¢≤·µƒ…Ë÷√ø®¿‡–Õ: `{t.FullName}`" };
            return BindMarkdown(err, existing);
        }
        ...
        private static void EnsureTextFont(UnityEngine.UI.Text t)
        {
            ...
            if (builtin != null) t.font = builtin; else
            {
        ...
        internal static GameObject BindNavigation(NavigationCardModel m, GameObject existing)
        {
            ...
            PositionTitleDesc(go, ResolveHeight(m)); PositionArrow(go, ResolveHeight(m));
        ...
        internal static GameObject BindMarkdown(MarkdownCardModel m, GameObject existing)
        {
            ...
            exp.TitleText = title; exp.ContentText = desc; exp.InitialHeight = ResolveHeight(m); exp.PaddingTop = padY; exp.PaddingBottom = padY; exp.TitleHeight = titleH; exp.PaddingX = padX;
        ...
        internal static GameObject BindAction(ActionCardModel m, GameObject existing)
        {
            ...
            PositionTitleDesc(go, ResolveHeight(m));
        ...
        private static float ResolveHeight(ICardModel m)
        {
            if (m.HeightOverride>0) return m.HeightOverride;
            if (m.HeightOverride<0) return Theme.ThemeMetrics.CardHeightMarkdown;
            switch(m.Size){ case CardSize.Small: return Theme.ThemeMetrics.CardHeightSmall; case CardSize.Large: return Theme.ThemeMetrics.CardHeightLarge; case CardSize.XLarge: return Theme.ThemeMetrics.CardHeightXLarge; default: return Theme.ThemeMetrics.CardHeightMedium; }
        }
