...
namespace IMK.SettingsUI.Table
{
    internal sealed class SchemaTableController : MonoBehaviour
    {
        ...
        private class Cell
        {
            ...
            // dropdown is not used in this simplified implementation
        ...
        public void Init(ITableSchema schema, ITableDataSet data, RectTransform viewport)
        {
            ...
            var vpImage = _viewport.GetComponent<Image>(); if (vpImage == null) { vpImage = _viewport.gameObject.AddComponent<Image>(); vpImage.color = new Color(0f, 0f, 0f, 0f); }
                ...
                for (int c = 0; c < _schema.Columns.Count; c++)
                {
                    ...
                    switch (col.Kind)
                    {
                        ...
                        case TableCellKind.Dropdown:
                            ...
                            break;
                        case TableCellKind.Toggle:
                            bool b = false; try { b = System.Convert.ToBoolean(val); } catch { }
                            if (cell.Toggle.isOn != b) cell.Toggle.isOn = b; cell.Toggle.onValueChanged.RemoveAllListeners(); if (!col.ReadOnly) cell.Toggle.onValueChanged.AddListener(v => { SelectIndex(idx); rv.Adapter.Set(col.Id, v); });
        ...
        private float GetTotalContentHeight() => HeaderHeight + (_data?.Count ?? 0) * RowHeight;
        ...
        void LateUpdate()
        {
            ...
            if (Mathf.Abs(wheel) > 0.01f)
            {
                if (RectTransformUtility.RectangleContainsScreenPoint(_viewport, Input.mousePosition))
                {
                    ...
                    float maxY = Mathf.Max(0f, GetTotalContentHeight() - _viewport.rect.height);
