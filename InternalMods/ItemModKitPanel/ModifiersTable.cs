    ...
    internal sealed class ModifiersDataSet : ITableDataSet
    {
        ...
        public bool Commit()
        {
            try
            {
                ...
                foreach (var o in _original) if (!string.IsNullOrEmpty(o.key)) originalMap[o.key] = o;

                // handle deletions
                foreach (var o in _original.Where(o => !string.IsNullOrEmpty(o.key)))
                {
                    if (_rows.Any(r => string.Equals(r.key, o.key, StringComparison.Ordinal))) continue;
