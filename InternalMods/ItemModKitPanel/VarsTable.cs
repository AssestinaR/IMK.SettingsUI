...
namespace IMK.SettingsUI.InternalMods.ItemModKitPanel
{
    internal static class VarTableCommon
    {
        ...
        internal static object ParseToType(string s, string type)
        {
            try
            {
                return type switch
                {
                    "int" => int.TryParse(s, out var i) ? i : 0,
                    "float" => float.TryParse(s, out var f) ? f : 0f,
                    "double" => double.TryParse(s, out var d) ? d : 0.0,
                    "bool" => bool.TryParse(s, out var b) ? b : false,
                    "long" => long.TryParse(s, out var l) ? l : 0L,
                    "short" => short.TryParse(s, out var sh) ? sh : (short)0,
                    "byte" => byte.TryParse(s, out var by) ? by : (byte)0,
                    _ => s ?? string.Empty
                };
            }
            catch { return s; }
    ...
    internal sealed class VarsDataSet : ITableDataSet
    {
        ...
        private readonly List<Row> _original = new();
        private bool _dirty;
        ...
        private sealed class VarsRowAdapter : IRowAdapter
        {
            ...
            public bool Set(string columnId, object value)
            {
                ...
                switch (columnId)
                {
                    case "scope": _r.scope = value?.ToString() ?? "Var"; break;
                    case "key": _r.key = value?.ToString() ?? string.Empty; break;
                    case "type": _r.type = value?.ToString() ?? "string"; break;
                    case "value": _r.value = value?.ToString() ?? string.Empty; break;
                    default: handled = false; break;
                }
                if (handled) _ds.OnChanged();
                return handled;
            }
        }
    }
    ...
    {
        private readonly List<(string key, string type, string value)> _rows = new();
        private List<(string key, string type, string value)> _original = new();
    ...
    internal sealed class TagsOnlyDataSet : ITableDataSet
    {
        ...
        private List<string> _original = new();
        public int Count => _rows.Count; public bool IsDirty { get; private set; }
        private object _item => ItemModKitPanelState.CapturedItem;
